...
module.exports = function(shipit) {
  ...
  shipit.db.dumpCmd = function dumpCmd(environment) {
    const credentials = shipit.config.db[environment];
    return shipit.config.db['adapter'] === 'postgres'
      ? `PGPASSWORD="${credentials.password}" pg_dump --no-owner --no-acl -U ${credentials.username} -h ${credentials.host} -p ${credentials.port} -F p ${credentials.database}`
      : sprintf('mysqldump %(credentials)s %(database)s --lock-tables=false %(ignoreTablesArgs)s', {
  ...
  shipit.db.createCmd = function createCmd(environment) {
    const credentials = shipit.config.db[environment];
    return shipit.config.db['adapter'] === 'postgres'
      ? `psql "postgresql://${credentials.username}:${credentials.password}@${credentials.host}:${credentials.port}/${credentials.database}" -c "drop schema public cascade; create schema public;"`
      : sprintf('mysql %(credentials)s --execute \"CREATE DATABASE IF NOT EXISTS %(database)s;\"', {
        credentials: shipit.db.credentialParams(shipit.config.db[environment]),
        database: shipit.config.db[environment].database,
      });
  };
  ...
  shipit.db.importCmd = function importCmd(environment, file) {
    const filepath = path.join(path.dirname(file), path.basename(file, '.bz2'));
    const credentials = shipit.config.db[environment];
    // `psql -U ${credentials.username} -h ${credentials.host} -p ${credentials.port} -d ${credentials.database} -1 -f ${filepath}`
    return shipit.config.db['adapter'] === 'postgres'
      ? `psql "postgresql://${credentials.username}:${credentials.password}@${credentials.host}:${credentials.port}/${credentials.database}" < ${filepath}`
      : sprintf('mysql %(credentials)s -D %(database)s < %(file)s', {
        credentials: shipit.db.credentialParams(shipit.config.db[environment]),
        database: shipit.config.db[environment].database,
  ...
  shipit.db.clean = function clean(environment, path, enabled) {
    ...
    var file = path
      .replace('.sql', '')
      .replace('.bz2', '') + '.*';
    return enabled ? shipit[environment]('rm -f ' + file) : Promise.resolve();
  };
