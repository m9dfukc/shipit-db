...
module.exports = function(shipit) {
  ...
  shipit.db.dumpCmd = function dumpCmd(environment) {
    ...
    return shipit.config.db[environment]['adapter'] === 'postgres'
      ? `PGPASSWORD="${credentials.password}" pg_dump -U ${credentials.username} -h ${credentials.host} -p ${credentials.port} -F p ${credentials.database}`
  ...
  shipit.db.createCmd = function createCmd(environment) {
    console.log(environment, shipit.config.db[environment]);
    const credentials = shipit.config.db[environment];
    return shipit.config.db[environment]['adapter'] === 'postgres'
      ? `psql -U ${credentials.username} -h ${credentials.host} -p ${credentials.port} -d ${credentials.database} -c "drop schema public cascade; create schema public;"`
