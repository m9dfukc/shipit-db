...
module.exports = function(shipit) {
  ...
  // overwrite shipit.db dump command to work with postgres
  shipit.db.dumpCmd = function dumpCmd(environment) {
    const credentials = shipit.config.db[environment];
    return `
      PGPASSWORD="${credentials.password}"
      pg_dump -U ${credentials.username} -h ${credentials.host} -p ${credentials.port} -FC ${credentials.database}`;
  };
  shipit.db.createCmd = function createCmd(environment) {
    const credentials = shipit.config.db[environment];
    return `psql -U ${credentials.username} -h ${credentials.host} -p ${credentials.port} -d ${credentials.database} -c "drop schema public cascade; create schema public;"`;
  };
  shipit.db.importCmd = function importCmd(environment, file) {
    const filepath = path.join(path.dirname(file), path.basename(file, '.bz2'));
    const credentials = shipit.config.db[environment];
    return `psql -U ${credentials.username} -h ${credentials.host} -p ${credentials.port} -d ${credentials.database} -1 -f ${filepath}`;
  };
